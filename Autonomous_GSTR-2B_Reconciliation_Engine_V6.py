...
# ============================================================
# AUTONOMOUS GSTR-2B RECONCILIATION ENGINE — V5.1
# Reconciliation Safety & Accuracy Hardened Edition
# ============================================================

st.set_page_config(
    page_title="GSTR-2B Auto-Reco V5.1",
    page_icon="📊",
    layout="wide"
)

st.title("⚡ Autonomous GSTR-2B Reconciliation Engine (V5.1)")
st.markdown("""
    ...
    for doc in doc_list:
        ...
        if items:
            for it in items:
                ...
                component_values["IGST"].append(_safe_float(d, "iamt"))
                component_values["CGST"].append(_safe_float(d, "camt"))
                component_values["SGST"].append(_safe_float(d, "samt"))
                component_values["CESS"].append(_safe_float(d, "csamt"))
            ...
            vals = {
                ...
                "IGST": _safe_float(doc, "iamt"),
                "CGST": _safe_float(doc, "camt"),
                "SGST": _safe_float(doc, "samt"),
                "CESS": _safe_float(doc, "csamt"),
            }
